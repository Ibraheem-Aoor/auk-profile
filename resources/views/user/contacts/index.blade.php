...
@endpush
@section('content')
    @include('user.partials.page_header', [
        'current_page_name' => __('general.contacts'),
        'sub_pages' => [
            __('general.contacts') => route('user.contacts.index'),
        ],
    ])
    {{-- Table Extra actions --}}
            ...
            <ul class="breadcrumb bg-transparent rounded mb-0 p-0">
                <li class="breadcrumb-item text-capitalize d-none">
                    <a class="btn btn-md btn-primary text-white m-1" data-bs-toggle="modal"
                        ...
                        href="{{ route('user.dashboard') }}">{{ __('general.new') }}</a>
                </li>
            </ul>
        </nav>
    </div>
...
@push('js')
    ...
    <script src="{{ asset('assets/user/js/user/contacts.js') }}"></script>
@endpush
